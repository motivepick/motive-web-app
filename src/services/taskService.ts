...
import { ICreateTaskRequest, ISearchScheduleWeekResponse, ISearchUserTasksResponse } from '../models/redux/taskServiceModel'

// eslint-disable-next-line @typescript-eslint/no-empty-function
const DO_NOTHING = () => {
}

const searchUserTasks = async (list: TaskListTypeAsLiterals, offset: number, limit: number): Promise<ISearchUserTasksResponse> => {
...
const updateTasksOrderAsync = (update: ITaskPositionIndex): Promise<void> =>
    request
        .post(`${API_URL}/orders`)
        .send(update)
        .withCredentials()
